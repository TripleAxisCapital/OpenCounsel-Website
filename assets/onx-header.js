// /assets/onx-header.js — optimized, mobile-first hero → pill
// Apple-grade, accessible, minimal DOM work, keeps all features you rely on.
...
/* Usage for ONX Pro page theme (page-scoped):
   ...
   Optional attrs: threshold="8"  (scroll px before shrinking to pill)
*/
...
class ONXHeader extends HTMLElement {
  ...
  connectedCallback() {
    ...
    this._root.innerHTML = `
      <style>
        :host{
          ...
          --pill-inner-x-mobile-initial: 30px; /* default before pill engages */
          --pill-inner-x-mobile: var(--pill-inner-x-mobile-initial);
          ...
          --mobile-hero-height: calc(100dvh - var(--safe-top) - var(--safe-bot));
        ...
        .center{ position:absolute; left:50%; transform:translateX(-50%); display:none; align-items:center; gap: var(--nav-gap); font-size: var(--nav-font-size); font-weight: var(--nav-font-weight); letter-spacing: var(--nav-letter-spacing); line-height: var(--nav-line-height); }
        .center .nav-link:not(.nav-link--black){
          background: linear-gradient(var(--angle,135deg), var(--grad-from), var(--grad-via), var(--grad-to));
          background-size: 300% 300%; animation: gradientShift var(--speed,16s) ease-in-out infinite;
          -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color: transparent; color: transparent; padding-bottom: .06em;
        }
        ...
        /* ===== MOBILE HERO (initial) → PILL (on scroll) ===== */
        :host(.mobile-hero) .oc-header{ padding-top: 0; padding-bottom: 0; }
        :host(.mobile-hero) .header-bar{
          ...
          padding-left: var(--logo-pad-left-mobile); padding-right: var(--download-pad-right-mobile);
        }
        /* When floating (pill) */
        ...
        @media (min-width:768px){ .desktop-actions .btn{ height: calc(var(--pill-height-desktop) - (2 * var(--download-btn-pad-y-desktop))); padding-block: var(--download-btn-pad-y-desktop); } }
        @media (min-width:768px){ .news-link{ display:inline-flex; } }
        ...
        /* ===== THEME: ONXPro (dark) ===== */
        :host([theme="ONXPro"]), :host([theme="onxpro"]), :host([theme="pro"]) {}
        ...
        :host([theme="pro-light"]) .header-bar{ background:#ffffff !important; color:#0A0D10 !important; border:none !important; box-shadow:none !important; border-radius: var(--header-radius); -webkit-backdrop-filter: none !important; backdrop-filter: none !important; }
        :host([invert].is-float) .header-bar,
        :host([theme="ONXProLight"].is-float) .header-bar,
        :host([theme="onxpro-light"].is-float) .header-bar,
        :host([theme="pro-light"].is-float) .header-bar{ background:#ffffff !important; border:none !important; box-shadow:none !important; }
        ...
        :host([theme="pro-light"]) .mobile-actions .btn{ background:#0A0D10 !important; color:#ffffff !important; animation:none !important; }
        ...
        :host([theme="pro-light"]) .logo-anim{ background:#000000 !important; animation:none !important; }
      </style>
      ...
      <div class="oc-header">
        <div class="header-bar">
          ...
          <nav class="center" aria-label="Primary">
            <!-- ONX Pro matches Pricing styling (no special black/bold) -->
    ...
    // Initial hero state on mobile, then scroll logic
    this._applyInitialHeroState();
  ...
  _renderExtraLinks(list = []) {
    ...
    list.forEach(item => {
      ...
      // Desktop link — ONX Pro matches Pricing (no forced black/bold)
      const a = document.createElement('a');
      a.className = 'nav-link' + (item.black && !isOnxPro ? ' nav-link--black' : '') + (item.pro && !isOnxPro ? ' nav-link--pro' : '');
      ...
      const p = document.createElementNS('http://www.w3.org/2000/svg','path'); p.setAttribute('d','m9 6 6 6-6 6'); chev.appendChild(p);
      m.appendChild(chev); m.addEventListener('click', () => this._toggleMobile(false));
  ...
  _cloneNow(name){
    ...
    if (name === 'nav'){
      ...
      assigned.forEach(node => {
        ...
        // Mobile clone — ONX Pro matches Pricing styling
        const a = document.createElement('a');
        ...
        if (centerExtra) {
          ...
          if (isOnxPro) d.classList.remove('nav-link--pro','nav-link--black');
  ...
  _onResize(){
    // On desktop, ensure menu closed and hero class removed
    if (window.matchMedia('(min-width: 768px)').matches){
      this._toggleMobile(false);
      this.classList.remove('mobile-hero');
    } else {
      // Re-apply hero on mobile when at top
      if ((window.scrollY || document.documentElement.scrollTop || 0) <= this._threshold) this.classList.add('mobile-hero');
    }
    this._syncEdgeGaps();
  }
  ...
  _onScroll() {
    ...
    if (isMobile){
      if (y > this._threshold){
        ...
        this.classList.add('mobile-hero');
        this.classList.remove('is-float');
      }
    } else {
      // Desktop: just toggle pill on threshold
  ...
  _applyInitialHeroState(){
    const isMobile = !window.matchMedia('(min-width: 768px)').matches;
    if (isMobile) this.classList.add('mobile-hero');
  }
  ...
  _syncEdgeGaps(){
    try{
      ...
      // Only compute fancy edge sync when in PILL state
      if (!this.classList.contains('is-float')){
        this.style.setProperty('--pill-inner-x-mobile', getComputedStyle(this).getPropertyValue('--pill-inner-x-mobile-initial') || '30px');
        this.style.setProperty('--logo-pad-left-mobile', '16px');
        this.style.setProperty('--download-pad-right-mobile', '16px');
        return;
      }
      ...
      const extra = 15; // tune this to micro-adjust side inset vs top gap
